  ...
  return <div className="grid min-h-screen grid-cols-1 bg-background font-sans text-foreground md:grid-cols-[206px_1fr]">
    <aside className="flex flex-col border-b border-[var(--line)] bg-background py-5 md:sticky md:top-0 md:h-screen md:border-r md:border-b-0 md:py-[22px]">
      ...
      <div className="mt-4 px-3.5 md:mt-auto md:px-2.5 md:pt-5">
        ...
        <div aria-live="polite" className="mt-2.5 hidden rounded-lg border border-[var(--line)] bg-[var(--panel)] px-3 py-3 shadow-[var(--shadow-card)] md:block" data-testid="telegram-sync-status">
          ...
          {sync?<dl className="mt-2.5 grid grid-cols-2 gap-x-3 gap-y-2 border-t border-[var(--line)] pt-2.5 font-mono"><div><dd className="text-[12px] font-medium tabular-nums text-foreground">{sync.received}</dd><dt className="mt-0.5 text-[8px] uppercase tracking-[0.07em] text-dim">received</dt></div><div><dd className="text-[12px] font-medium tabular-nums text-foreground">{sync.synced}</dd><dt className="mt-0.5 text-[8px] uppercase tracking-[0.07em] text-dim">synced</dt></div><div><dd className="text-[12px] font-medium tabular-nums text-foreground">{sync.processing}</dd><dt className="mt-0.5 text-[8px] uppercase tracking-[0.07em] text-dim">processing</dt></div><div><dd className="text-[12px] font-medium tabular-nums text-foreground">{sync.failed}</dd><dt className="mt-0.5 text-[8px] uppercase tracking-[0.07em] text-dim">failed</dt></div></dl>:<div className="mt-2.5 border-t border-[var(--line)] pt-2.5 font-mono text-[9px] leading-relaxed text-dim">Waiting for current status</div>}
        </div>
