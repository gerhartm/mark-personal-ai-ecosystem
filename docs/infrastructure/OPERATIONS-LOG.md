...
- Status: production origin completed; root-domain Cloudflare Access cutover pending a new short-lived credential.
...
- Domain boundary: no root DNS or tunnel route has been created yet, so the origin cannot be reached publicly without operator access. Existing `brain`, `manage`, `manage-realtime`, and legacy `intel` hostnames remain unchanged.
