...
**Status:** secured base server online; Coolify 4.2.0, Hermes Agent 0.19.1, private OpenViking 0.4.11, ForkedBrain release `20260805T152846Z`, Crypto Intelligence release `20260805T152846Z`, and isolated V2 Cloudflare tunnel healthy
...
The normalized dashboard database, private 89-file media archive, and complete 197-record OpenViking semantic import are complete, owner-only, and independently verified on the VPS. ForkedBrain and Crypto Intelligence release `20260805T152846Z` are healthy, restart-safe, loopback-only, and connected to the funded native Anthropic provider through Hermes. The ForkedBrain Crypto branch now routes to `crypto.forkedbrain.fyi`; the Memory Graph logo and Back control return to the main brain overview; and the Crypto Intelligence logo returns to its dashboard home. The sourced daily command center, Ask, evidence-linked Quiz, Content Studio, Speaking Preparation, Capture, and Library are deployed; OpenViking's native memory providers remain available. `crypto.forkedbrain.fyi` is live behind the existing exact-email Access application and isolated V2 tunnel. Crypto has not replaced or changed the legacy `intel.forkedbrain.fyi` service. Darshan confirmed an interactive owner login, and the short-lived Crypto cutover token independently verifies as revoked or invalid. The post-import OpenViking recovery point is `/root/mark-v2-backups/20260805T112800Z-post-complete-crypto-import/`; the current dashboard rollback point is `/srv/mark-v2/crypto-dashboard/backups/pre-20260805T152846Z/crypto-intelligence.db` with stopped predecessor `crypto-dashboard-rollback-20260805T143933Z`.
