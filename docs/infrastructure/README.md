...
The complete legacy Crypto dashboard handoff is staged privately on the VPS: an integrity-verified normalized database plus 89 checksum-verified transcript/media files. ForkedBrain now runs as a separate hardened, non-root Next.js container with a read-only copy of that database, a loopback-only origin at `127.0.0.1:9320`, a dynamic graph capped at 20 nodes, retained research conversations, real detail/provenance views, and native Hermes selected-memory chat transport. It adds no database, memory provider, or reasoning layer. Its root-domain Cloudflare Access and tunnel route remain the final external cutover step.
