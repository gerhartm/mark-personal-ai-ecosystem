...
**Status:** secured base server online; Coolify 4.2.0, Hermes Agent 0.19.1, private OpenViking 0.4.11, ForkedBrain release `20260805T152846Z`, Crypto Intelligence release `20260806T052651Z`, and isolated V2 Cloudflare tunnel healthy
...
The complete legacy Crypto dashboard handoff is staged privately on the VPS: an integrity-verified normalized database plus 89 checksum-verified transcript/media files. ForkedBrain runs as a separate hardened, non-root Next.js container with a read-only copy of that database, a loopback-only origin at `127.0.0.1:9320`, a dynamic graph capped at 20 nodes, retained research conversations, real detail/provenance views, and native Hermes selected-memory chat transport. The dedicated Crypto Intelligence dashboard runs separately at loopback-only `127.0.0.1:9330`, uses the canonical 66-event/47-source database and private media archive, sends bounded evidence to Hermes for answers, and exposes an authenticated URL/text Capture screen over OpenViking's native resource path. The Capture layer adds only stable identity, duplicate prevention, receipts, and local registration after native acceptance. Both applications add no model-provider key, memory provider, reasoning layer, scraper, queue, or vector store. `https://forkedbrain.fyi/` and `https://crypto.forkedbrain.fyi/` are live behind the exact-email Access boundary.

The complete semantic migration is accepted in native OpenViking as 131 source/artifact packets plus 66 event packets. Exact reconciliation found all 197 deterministic targets, an independent replay skipped all 197 with zero creates or failures, six representative deep reads preserved provenance, and production Crypto Ask returned four canonical citations that all resolved. OpenViking uses Luna only for native high-volume resource processing and keeps `text-embedding-3-small` for vectors; Hermes remains on its funded native Anthropic provider. The checksum-verified post-import volume archive passed a disposable restore test. No Telegram token has been configured.

The Crypto Brief is now a Hermes-powered daily intelligence command center with a sourced review, attention items, changes, watchlists, suggested actions, strongest stored signals, recent work, and system activity. It links directly into Ask, evidence-linked Quiz, Content Studio, Speaking Preparation, Capture, and Library. Hermes uses its official key-free DDGS web-search backend. Reviews are bounded to a 24-hour automatic cadence, manual refresh is rate-limited, and the last accepted brief remains available if a later run fails.
...
The normalized dashboard database, private 89-file media archive, and complete 197-record OpenViking semantic import are complete, owner-only, and independently verified on the VPS. ForkedBrain release `20260805T152846Z` and Crypto Intelligence release `20260806T052651Z` are healthy, restart-safe, loopback-only, and connected to the funded native Anthropic provider through Hermes. The ForkedBrain Crypto branch routes to `crypto.forkedbrain.fyi`; the Memory Graph logo and Back control return to the main brain overview; and the Crypto Intelligence logo returns to its dashboard home. The sourced daily command center, Ask, evidence-linked Quiz, Content Studio, Speaking Preparation, Capture, and Library are deployed. Studio exposes Mark and Creator Reference writing lenses through native Hermes skills while all contexts continue to use the same OpenViking memory. Telegram topic activation remains intentionally gated until a Mark-owned bot has topics enabled and its old poller is confirmed stopped. `crypto.forkedbrain.fyi` remains behind the existing exact-email Access application and isolated V2 tunnel. Crypto has not replaced or changed the legacy `intel.forkedbrain.fyi` service. The post-import OpenViking recovery point is `/root/mark-v2-backups/20260805T112800Z-post-complete-crypto-import/`; the current dashboard rollback point is `/srv/mark-v2/crypto-dashboard/backups/pre-20260806T052651Z/crypto-intelligence.db` with stopped predecessor `crypto-dashboard-rollback-20260805T152846Z`.
